...
chamber_1_start = 0.0101484
chamber_1_end = 0.0128616
chamber_2_start = 0.0172734
chamber_1_start_parametric = fishy.project(points=np.array([[chamber_1_start, 0., 0.]]), grid_search_density_parameter=10., plot=False)
chamber_1_end_parametric = fishy.project(points=np.array([[chamber_1_end, 0., 0.]]), grid_search_density_parameter=10., plot=False)
chamber_width = chamber_1_end - chamber_1_start
distance_between_chambers = chamber_2_start - chamber_1_end
num_chambers = 9
ffd_chamber_sections = []
for i in range(num_chambers):
    ffd_chamber_sections.append(chamber_1_start + i*distance_between_chambers)
    ffd_chamber_sections.append(chamber_1_end + i*distance_between_chambers)
ffd_x_values = [ffd_min_x] + ffd_chamber_sections + [ffd_max_x]
ffd_x_values = np.array(ffd_x_values)
# chamber_2_end = 0.0199766
# print(chamber_1_end + distance_between_chambers + chamber_width)
...
# chamber_width_delta = csdl.Variable(value=0., name='chamber_width_delta')
chamber_width_delta = csdl.Variable(value=0.000, name='chamber_width_delta')
...
chamber_start_section_indices = list(np.arange(1, ffd_block.coefficients.shape[0]-1, 2))
chamber_end_section_indices = list(np.arange(2, ffd_block.coefficients.shape[0]-1, 2))
ffd_block_coefficients = ffd_block_coefficients.set(csdl.slice[chamber_start_section_indices,:,:,0], 
                                                    ffd_block_coefficients[chamber_start_section_indices,:,:,0] - chamber_width_delta)
ffd_block_coefficients = ffd_block_coefficients.set(csdl.slice[chamber_end_section_indices,:,:,0], 
                                                    ffd_block_coefficients[chamber_end_section_indices,:,:,0] + chamber_width_delta)

fishy_coefficients = ffd_block.evaluate(coefficients=ffd_block_coefficients, plot=True)
...
computed_chamber_width = csdl.norm(fishy.evaluate(chamber_1_end_parametric) - fishy.evaluate(chamber_1_start_parametric))
...
chamber_width = csdl.Variable(value=computed_chamber_width.value, name='chamber_width')
# chamber_width = csdl.Variable(value=0.004, name='chamber_width')
...
geometry_parameterization_solver.add_parameter(chamber_width_delta)
...
geometric_parameterization_variables.add_variable(computed_chamber_width, chamber_width)
...
# width_shape_variables.set_as_design_variable(lower=-0.01, upper=0.01, scaler=1.e2)
# width_shape_variables.set_as_design_variable(lower=-0.01, upper=0.01, scaler=1.e2)
# height.set_as_design_variable(lower=0.03, upper=0.14, scaler=1.e2)
chamber_width.set_as_design_variable(lower=0.002, upper=0.004, scaler=1.e3)
