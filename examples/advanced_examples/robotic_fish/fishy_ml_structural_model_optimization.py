...
Design variables: 
# 1. Length
2. Change in Width profile
3. Height
4. Actuation Frequency
5. Pump Max Pressure
...
Subject to: 
# 1. Length <= 0.7
2. Change in width <= 40% of initial width
3. Height <= 200% initial height, Height >= 80% initial height
4. Actuation frequency <= 1., Actuation frequency >= 0.2
5. Pump Max Pressure <= 5e4, Pump Max Pressure >= 5.e3
6. Pump Max Pressure <= 3.5e4 at 1 Hz, Pump Max Pressure <= 5e4 at 0.5 Hz, linearly interpolated inequality constraint
...
#             training_data.append(sample_displacement)
#         training_data = np.hstack(training_data)
#         return training_data

# # -- Import data from data_5_dims.hdf5
# training_data_5_dims = import_training_data('saved_data_5_dims.hdf5')
# training_data_6_dims = import_training_data('saved_data_6_dims.hdf5')
# training_data = np.hstack([training_data_5_dims, training_data_6_dims])

# # -- Compute POD Basis
# required_energy_percentage = 0.9999
# U, S, V_T = np.linalg.svd(training_data, full_matrices=False)
# total_energy = S.T.dot(S)
# kept_energy_percentage = np.cumsum(S**2)/total_energy
# num_kept_singular_values = np.where(kept_energy_percentage > required_energy_percentage)[0][0] + 1
# P = U[:,:num_kept_singular_values]      # POD Basis
# print('Number of kept singular values: ', num_kept_singular_values)
# print('Original Degrees of Freedom: ', training_data.shape[0])
# print('Reduction Percentage: ', num_kept_singular_values/training_data.shape[0])
# print('Kept energy percentage: ', kept_energy_percentage[num_kept_singular_values-1])

# import matplotlib.pyplot as plt
# plt.semilogy(S, '-bo')
# plt.semilogy(num_kept_singular_values, S[num_kept_singular_values], 'rx')
# plt.show()

# # -- Save POD Basis
# np.save('POD_Basis_9999.npy', P)

# # -- Check POD Basis
# P = np.load('POD_Basis_9999.npy')
# rank_reduced_structural_displacements = P.dot(P.T.dot(structural_displacements.value.reshape((-1,1))))
# error = np.linalg.norm(structural_displacements.value.reshape((-1,1)) - rank_reduced_structural_displacements)
# print('Error: ', error)

# # endregion Construct POD Basis
...
# jax_inputs = [height, width_shape_variables]
# additional_outputs = []
# for config in fishy_configurations:
#     additional_outputs.append(config.coefficients)
    
# plotting_sim = csdl.experimental.JaxSimulator(
#     recorder = recorder,
...
# import vedo
# video = vedo.Video('examples/advanced_examples/robotic_fish/temp/dynamic_geometry.mp4', fps=num_steps_per_cycle/2, backend='opencv')
# for i in range(time.shape[0]):
#     fishy_configuration_plot = fishy_configurations[i].plot(show=False)
#     plotter = vedo.Plotter(axes=0, offscreen=True)
#     plotter.show(fishy_configuration_plot, viewup='y')
#     video.add_frame()
# video.close()
# exit()
...
# swim_speed = csdl.Variable(value=0.21791964)
...
output_dict, panel_mesh_dict, wake_panel_mesh_dict, mu, sigma, mu_wake = unsteady_panel_solver(
    ...
    coll_vel_list,
    dt=dt, # leads to more consistent scaling on the order of 1e-2 I think
    free_wake=False
)
...
# net_thrust_drag = csdl.sum(panel_forces[0,:-1,:,:,0]) # leads to scaling on the order of 1 I think
panel_forces_output = csdl.sum(panel_forces[0,:-1,:,:,0], axes=(1,2))   # leads to more consistent scaling on the order of 1e-1 I think
net_thrust_drag = csdl.average(panel_forces_output)     # leads to more consistent scaling on the order of 1e-1 I think
# net_thrust_drag = csdl.average(panel_forces[0,:-1,:,:,0])   # leads to more consistent scaling on the order of 1e-2 I think
...
# alpha = 1.
alpha = csdl.Variable(value=0.)
width_shape_regularization_penalty_factor = 1.e2
# average_width_change = csdl.average(width_shape_variables)
width_delta_changes = width_shape_variables[1:] - width_shape_variables[:-1]
quadratic_regularization = csdl.vdot(width_delta_changes, width_delta_changes)
width_shape_regularization = width_shape_regularization_penalty_factor*quadratic_regularization
# width_shape_regularization = width_shape_regularization_penalty_factor*csdl.average(width_shape_variables - average_width_change)**2
# objective = alpha*cost_of_transport + (1 - alpha)*(-swim_speed) + width_shape_regularization + 1e6*net_thrust_drag**2
# objective = alpha*cost_of_transport + (1 - alpha)*(-swim_speed) + 1e6*net_thrust_drag**2
# objective = alpha*cost_of_transport + (1 - alpha)*(-swim_speed) + 1e4*net_thrust_drag**2
# objective = -swim_speed
# objective = cost_of_transport + width_shape_regularization + 1e3*net_thrust_drag**2
# objective = cost_of_transport + width_shape_regularization + 1e6*net_thrust_drag**2
# objective = cost_of_transport + width_shape_regularization + 1e1*net_thrust_drag**2
# objective = -0.2*swim_speed + 0.9*cost_of_transport
objective = cost_of_transport
# objective = cost_of_transport + 1.e6*net_thrust_drag**2
# objective = cost_of_transport + 1.e4*net_thrust_drag**2   # This converged for the 4dv optimization
# objective = cost_of_transport + 1.e2*net_thrust_drag**2
...
# # region Optimization
# swim_speed.set_as_design_variable(lower=0.05, upper=0.5, scaler=1.e1)
actuation_frequency.set_as_design_variable(lower=0.5, upper=1., scaler=1.e1)
# pump_pressure.set_as_design_variable(lower=5.e3, upper=4.5e4, scaler=1e-4)
pump_pressure.set_as_design_variable(lower=5.e3, upper=3.75e4, scaler=1e-4)
...
# width_shape_variable.set_as_design_variable(lower=-computed_fishy_width.value*0.4/2, upper=computed_fishy_width.value*0.4/2, scaler=1.e3)
width_shape_variables.set_as_design_variable(lower=-(computed_fishy_width.value*0.4)/2, upper=(computed_fishy_width.value)/2, scaler=1.e3)
...
# net_thrust_drag.set_as_constraint(equals=0, scaler=1.e0)
net_thrust_drag.set_as_constraint(equals=0, scaler=1.e1)
...
# objective.set_as_objective(scaler=1.e-1)

# region Set Initial Guesses
# # swim_speed.set_value(2.179196355846669597e-1)
# actuation_frequency.set_value(5.046258421911439029e-1)
# pump_pressure.set_value(3.515244588904933742e4)
# width_shape_variables.set_value(5*np.ones(4)*1.e-3)
# height.set_value(9.427664330455034047e-2)
# # width_shape_variables.set_value(np.array([-6.588304931104882023e-3, -6.164310386667808395e-3, -7.726746867260755103e-3, -8.026507042893783250e-3]))
# # height.set_value(7.427231443560996738e-2)

# Manual guess
# swim_speed.set_value(1.241627638045792104e-1)
# actuation_frequency.set_value(2.875264263369016415e-1)
# pump_pressure.set_value(3.156550884600514095e+4)
# width_shape_variables.set_value(np.array([3.209882008121137886e-3, -7.898648460233428814e-4, -3.750942413672723852e-3, -7.653192328209678763e-3]))
# height.set_value(1.332272358366183518e-1)


# Better converged speed maximization result
# swim_speed.set_value(1.141627638045792104e-1)
# actuation_frequency.set_value(2.875264263369016415e-1)
# pump_pressure.set_value(3.356550884600514095e+4)
# width_shape_variables.set_value(np.array([-3.209882008121137886e-3, -7.898648460233428814e-3, -3.750942413672723852e-3, -7.653192328209678763e-3]))
# height.set_value(1.332272358366183518e-1)


# Converged speed maximization result
# swim_speed.set_value(5.127460725115433604e-02)
# actuation_frequency.set_value(2.093936406670147576e-1)
# pump_pressure.set_value(3.155788890427329463e+4)
# width_shape_variables.set_value(np.array([-7.847762271641366638e-3, -7.938202191179947853e-3, -5.614391796077312868e-3, -3.632105381263753152e-3]))
# height.set_value(1.333403176910041665e-1)


# Didn't quite converge but was kinda close and was only pressurized on one side
# swim_speed.set_value(1.001271565268009756e-1)
# actuation_frequency.set_value(6.395649200961899261e-1)
# pump_pressure.set_value(2.666048860408992915e4)
# width_shape_variables.set_value(np.array([-4.339403921715438983e-3, 8.026504313300508286e-3, -2.610824650632309396e-4, 4.051397532792812561e-4]))
# height.set_value(9.056532042355675216e-2)

# Converged solution with new parameters, but sloooow swim speed, also just realized only one side was pressurized
# swim_speed.set_value(1.008025941307171824e-02)
# actuation_frequency.set_value(5.167476804430155113e-01)
# pump_pressure.set_value(6.415939127551187893e+03)
# width_shape_variables.set_value(np.array([7.478131212004761608e-03, 2.129669526081777509e-03, -2.406829439270033733e-04, -2.822097838493387578e-04]))
# height.set_value(1.204864298932126765e-01)

# Manual guess at optimal control for baselin design
# actuation_frequency.set_value(0.13*2)
# pump_pressure.set_value(2.830284399072577894e4)

# # # My manual guess that led to the rectangular design
# actuation_frequency.set_value(7.292348947543857207e-1)
# # pump_pressure.set_value(2.686004316014740212e+4)
# pump_pressure.set_value(3.7e+4)    # replacing with manual guess
# width_shape_variables.set_value(np.array([3.733458334257954370e-3, -6.539584245001941021e-3, -8.026507043130138186e-3, -8.026507043130138186e-3]))
# height.set_value(5.581799618454310874e-2)
# # height.set_value(5.381799618454310874e-2)

# Optimal design with newest model parameters, speed set ot 0.66 body lengths per second, cost of transport min
# actuation_frequency.set_value(8.359115778113318740e-1)
# pump_pressure.set_value(2.245379881538228428e+4)
...
# Optimal design from dynamic beam optimization. This is just to generate the cross section for paraview
pump_pressure.set_value(2.980301246807620874e+4)    # divided by 10
actuation_frequency.set_value(7.671249518924843080e-1)
...
# Once again, my manual guess for a hopefully more rectangular design with speed of 0.13 m/s
# actuation_frequency.set_value(8.458856057810814733e-1)
# pump_pressure.set_value(5.e3)
# width_shape_variables.set_value(np.array([3.606052686277899433e-3, -7.611414918586879352e-3, -8.026507043130061803e-3, -8.026507043130084895e-3]))
# height.set_value(4.003723402530887654e-2)

# Optimal design with set speed of .66 body lengths per second and new model parameters and me setting inital guess to influence rectangular output 
# actuation_frequency.set_value(7.295532570502395231e-1)
# pump_pressure.set_value(3.663998499292308253e+4)
# width_shape_variables.set_value(np.array([3.940879315942797056e-3, -6.679241910280764749e-3, -7.884651385865020679e-3, -8.019684161629482944e-3]))
# height.set_value(5.592554057812760604e-2)

# # Optimal design with set speed of .66 body lengths per second and new model parameters
# actuation_frequency.set_value(7.638728038510197216e-1)
# pump_pressure.set_value(3.725135992195088619e+4)
# width_shape_variables.set_value(np.array([-7.872819610046351890e-3, -7.166624552740931797e-3, -8.006125571297896926e-3, -2.609224441192827371e-3]))
# height.set_value(5.376835074163169637e-2)

# # Optimal design with 0.2 swim speed and new model parameters
# actuation_frequency.set_value(7.249861717717720921e-1)
# pump_pressure.set_value(2.913348147263009835e+4)
# width_shape_variables.set_value(np.array([3.889501470861717713e-3, -8.023243959472468489e-3, -8.026264080534691203e-3, -8.021582916813574826e-3]))
# height.set_value(5.338401046431091146e-2)


# # Optimal design with set speed of .66 body lengths per second
# actuation_frequency.set_value(7.156040965234843121e-1)
# pump_pressure.set_value(2.472724806605190562e+4)
# width_shape_variables.set_value(np.array([5.731138044206841364e-3, -6.657115938444321479e-3, -7.965134789907922119e-3, -8.002576986651789070e-3]))
# height.set_value(7.931587973291041926e-2)

# Optimal design and control with only 1 width dv and set speed of .66 body lengths per second
# actuation_frequency.set_value(7.084463502809089874e-1)
# pump_pressure.set_value(2.672739422033396650e+4)
# width_shape_variable.set_value(-6.373962711288758243e-3)
# height.set_value(8.335187854197862123e-2)

# Optimal control for initial design
# actuation_frequency.set_value(6.785371515841888801e-1)
# pump_pressure.set_value(2.830284399072577894e4)

# Baseline control for initial design
# actuation_frequency.set_value(0.5)
# pump_pressure.set_value(4.5e4)

# endregion Set Initial Guesses
...
additional_inputs = [width_shape_variables, height, actuation_frequency, pump_pressure, swim_speed, alpha]
# additional_inputs = [width_shape_variable, height, actuation_frequency, pump_pressure, swim_speed]
# additional_inputs = []
...
print('swim speed: ', swim_speed.value)
print('cost of transport: ', cost_of_transport.value)
print('real objective: ', 0.5*cost_of_transport.value + 0.5*(-swim_speed.value))
exit()

# sim[swim_speed] = 2.179196355846669597e-1
# sim[actuation_frequency] = 5.046258421911439029e-1
# sim[pump_pressure] = 3.515244588904933742e4
# sim[width_shape_variables] = np.array([-6.588304931104882023e-3, -6.164310386667808395e-3, -7.726746867260755103e-3, -8.026507042893783250e-3])
# sim[height] = 7.427231443560996738e-2

# print('------------------*************------------------')
# sim.run()
# t3 = time.time()
# print('compile and run time: ', t3 - t2)
# print('constraint value', net_thrust_drag.value)
# print('objective value', objective.value)
# print('swim speed: ', swim_speed.value)
# print('cost of transport: ', cost_of_transport.value)
...
# width_shape_variable_sweep = np.linspace(-0.01, 0.01, 41)
# pump_pressure_sweep = np.linspace(1.e3, 4.e4, 5)
# net_thrust_drags = []
# cost_of_transports = []
...
#     # sim[width_shape_variable] = width_shape_variable_sweep[i]
#     # sim[pump_pressure] = pump_pressure_sweep[i]
#     sim.run()
...
# import matplotlib.pyplot as plt
# plt.figure(1)
# plt.plot(width_shape_variable_sweep, np.array(net_thrust_drags), '-x')
# plt.figure(2)
# plt.plot(width_shape_variable_sweep, np.array(cost_of_transports), '-o')
# plt.show()
# exit()
...
optimizer = PySLSQP(optimization_problem, solver_options={'maxiter':100, 'acc':1.e-3}, readable_outputs=['x'])
...
# # Sweep over optimizations
# alphas = np.linspace(1., 0., 5)
# swim_speed_outputs = []
# cost_of_transport_outputs = []
# for i in range(len(alphas)-1):
#     alpha_value = alphas[i+1]
#     # alpha.value = alpha_value
#     alpha.set_value(alpha_value)
#     sim[alpha] = alpha_value
#     optimizer.problem.options['simulator'][alpha] = alpha_value
...
#     optimizer.print_results()
#     # recorder.execute()    # I ran out of memory at this part

#     objective_value = recorder.objectives[objective]
#     cost_of_transport_value = objective_value + alpha_value*swim_speed.value

#     swim_speed_outputs.append(swim_speed.value)
#     cost_of_transport_outputs.append(cost_of_transport_value)
...
#     print('swim speed: ', swim_speed.value)
#     print('cost of transport: ', cost_of_transport.value)
#     # print('real objective: ', alpha_value*cost_of_transport.value + (1-alpha_value)*(-swim_speed.value))
#     print('------------------*************------------------')


# import matplotlib.pyplot as plt
# plt.figure(1)
# plt.plot(swim_speed_outputs, cost_of_transport_outputs)
# plt.show()
