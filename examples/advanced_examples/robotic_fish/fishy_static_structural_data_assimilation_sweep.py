...
plt.title('Bending Angle vs Pressure for Baseline Design')
...
# # Optimized set of values for optimized design
...
# Optimized set of values for both data (supposedly)
# E_dragon.set_value(2.608265819840872801e+5)
# nu_dragon.set_value(3.000000000000000000e-1)
# E_fr4.set_value(5.833226413830866486e+7)

# What I've been using for optimization
# E_dragon.set_value(2.3794898134622741e+5)
# nu_dragon.set_value(0.3)
# E_fr4.set_value(0.223121251595327195e+9)
# nu_fr4.set_value(0.12)

# Initial values for Robosoft paper
E_dragon.set_value(0.22e6)
nu_dragon.set_value(0.499)
E_fr4.set_value(3.7e6)
nu_fr4.set_value(0.12)


# sim = csdl.experimental.PySimulator(recorder=recorder)
additional_inputs = [length, height, pump_pressure, E_dragon, nu_dragon, E_fr4, nu_fr4, width_shape_variables]
additional_outputs = [objective, angle]
...
# # region copying result from prevous run
# bending_angles = [0., 1.4138908,  3.0758529,  4.8345539,  6.6663553,  8.5685257, 10.5449913, 12.6029909, 14.7514094, 16.9987394, 
#                   19.3506851, 21.8096948, 24.3780823, 27.0590041, 29.8536388, 32.7857309, 36.71960275, 41.06316607]
# bending_angles = [0., 1.4138908,  3.0758529,  4.8345539,  6.6663553,  8.5685257, 10.5449913, 12.6029909, 14.7514094, 16.9987394, 
#                   19.3506851, 21.8096948, 24.3780823, 27.0590041, 29.8536388]   # dummy output to skip initial sweep
# pump_pressures_for_plot = pump_pressures
# # endregion copying result from prevous run
...
# ^ UNCOMMENT WHEN NOT LOADING FROM PREVIOUS RUN

print('bending_angles with initial parameters: ', bending_angles)

import matplotlib.pyplot as plt
plt.plot(np.array(pump_pressures_for_plot)/1e3, bending_angles, '-o', label='Simulated with Initial Parameters', color='red')


# compute RMSE against experimental data for bending angles
differences = bending_angles_data - bending_angles.reshape((-1,1))
rmse_initial = np.sqrt(np.mean(differences**2))
print('RMSE initial: ', rmse_initial)

# Updated values for Robosoft paper
E_dragon.set_value(0.25e6)
nu_dragon.set_value(0.42)
E_fr4.set_value(0.8e9)
nu_fr4.set_value(0.12)
sim[E_dragon] = 0.25e6
sim[nu_dragon] = 0.42
sim[E_fr4] = 0.8e9
...
bending_angles = bending_angles - bending_angles[0]
bending_angles = bending_angles*180/np.pi*2   # convert to degrees and multiply by 2 for symmetry
print('bending_angles with updated parameters: ', bending_angles)


import matplotlib.pyplot as plt
plt.plot(np.array(pump_pressures_for_plot)/1e3, bending_angles, '-s', label='Simulated with Updated Parameters', color='orange')


# compute RMSE against experimental data for bending angles
differences = bending_angles_data - bending_angles.reshape((-1,1))
rmse_updated = np.sqrt(np.mean(differences**2))
print('RMSE final: ', rmse_updated)
print('RMSE initial: ', rmse_initial)
