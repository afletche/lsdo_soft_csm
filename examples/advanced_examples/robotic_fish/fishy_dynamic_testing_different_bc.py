...
# input_function_space = FunctionSpace(mesh, ('DG', 0))   # For some reason, finding the dofs only works if the input function space is CG2
...
E = 6.e5  # Young's modulus of dragon skin 30 silicone rubber (Pa) NOTE: Listed as 100% modulus on data sheet
...
fea.add_strong_bc(ubc_1, [front_wall_dofs])    # This is the correct way to do it
...
def dynamic_solve(residual_form, u, ubc, report=False):
    ...
    for ti in t:
        ...
        pressure_input.x.array[left_chamber_facet_dofs] = -left_chamber_pressure[time_step_index]
        pressure_input.x.array[right_chamber_facet_dofs] = -right_chamber_pressure[time_step_index]
