...
model = csdml.FCNN(input_dim=input_size, hidden_dims=[160, 160, 160], output_dim=output_size, activation=['relu', 'relu', 'relu', None])

optimizer = optax.adam(1e-3)
loss_history, test_loss_history, best_param_vals = model.train_jax_opt(optimizer, loss_data, test_data=test_data, 
                                                                       device=device, num_epochs=200, num_batches=1, plot=False)
# save loss history
