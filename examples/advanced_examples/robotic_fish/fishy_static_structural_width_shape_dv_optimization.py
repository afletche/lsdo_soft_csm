...
plt.plot(pressures[:,0], bending_angles[:,0], 'rx', label='Experimental Data')
plt.plot(pressures, bending_angles[:,1:], 'rx')
plt.plot(pressures, average_bending_angles, '-', linewidth=7., label='Mean (Experimental Data)')
plt.fill_between(pressures[:,0],
                 average_bending_angles-bending_angles_standard_deviation, average_bending_angles+bending_angles_standard_deviation,
                 alpha=0.3, label='1 Standard Deviation (Experimental Data)')
plt.title('Bending Angle vs Pressure')
...
coefficients_parametric = fishy.project(points=fishy.coefficients, grid_search_density_parameter=5., plot=False, force_reproject=False)
...
additional_inputs = [length, width, height, pump_pressure]
additional_outputs = [objective, angle, displaced_mesh]
...
import matplotlib.pyplot as plt
plt.plot(np.array(pump_pressures_for_plot), np.array(bending_angles)*180/np.pi*2, '-go', label='Simulated')
plt.legend()
plt.show()
exit()
